...
async function generateReport(param) {
	...
	var apiurl = `${global.modulefullname}/get_araging_bybrand`
	var args = {
		options: {
			criteria: {
				date: '2021-09-30'
	...
	if (param.filter_id!='ALL') {
		args.options.criteria.brand_id = param.filter_id;
	}
	...
	try {
		...
		for (var record of result.records) {
			var brand_name = record.brand_name;
			if (brand_name!=last_brand_name) {
				if (c>0) {
					// summary
					$(`
					<tr>
				...
				$(`
				<tr>
					<td class="col_group_head" colspan="12">${record.brand_name}</td>
				</tr>
				`).appendTo(obj_reporttable_body);
			...
			$(`
				<tr>
					...
					<td class="rptrow col_content">${record.advertiser_partner_name}</td>
