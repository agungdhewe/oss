...
const tbl_list = $('#pnl_editmst_collectorpartnergrid-tbl_list');
const txt_title = $('#pnl_editmst_collectorpartnergrid-title');
const pnl_control = $('#pnl_editmst_collectorpartnergrid-control');
const btn_removechecked  = $('#pnl_editmst_collectorpartnergrid-removechecked');
const btn_addrow = $('#pnl_editmst_collectorpartnergrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.collector_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/mst_collectorpartner-list`
		options.criteria['id'] = data.collector_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmst_collectorpartnerform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmst_collectorpartnerform', () => {
		$ui.getPages().ITEMS['pnl_editmst_collectorpartnerform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/mst_collectorpartner-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmst_collectorpartnerform', ()=>{
		$ui.getPages().ITEMS['pnl_editmst_collectorpartnerform'].handler.createnew(header_data)
	})	
}