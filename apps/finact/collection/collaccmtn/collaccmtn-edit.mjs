...
const obj = {
	txt_collaccmtn_id: $('#pnl_edit-txt_collaccmtn_id'),
	cbo_periodemo_id: $('#pnl_edit-cbo_periodemo_id'),
	dt_collaccmtn_date: $('#pnl_edit-dt_collaccmtn_date'),
	...
	cbo_collfuptype_id: $('#pnl_edit-cbo_collfuptype_id'),
	txt_collaccmtn_descr: $('#pnl_edit-txt_collaccmtn_descr'),
	txt_collaccmtn_notes: $('#pnl_edit-txt_collaccmtn_notes'),
	txt_collaccmtn_response: $('#pnl_edit-txt_collaccmtn_response'),
	dt_collaccmtn_nextactdate: $('#pnl_edit-dt_collaccmtn_nextactdate'),
	txt_collaccmtn_nextactnotes: $('#pnl_edit-txt_collaccmtn_nextactnotes'),
	txt_collaccmtn_version: $('#pnl_edit-txt_collaccmtn_version'),
	chk_collaccmtn_iscommit: $('#pnl_edit-chk_collaccmtn_iscommit'),
	txt_collaccmtn_commitby: $('#pnl_edit-txt_collaccmtn_commitby'),
	txt_collaccmtn_commitdate: $('#pnl_edit-txt_collaccmtn_commitdate')
}
...
export async function init(opt) {
	...
	form = new global.fgta4form(pnl_form, {
		primary: obj.txt_collaccmtn_id,
		autoid: true,
		logview: 'trn_collaccmtn',
	...
	new fgta4slideselect(obj.cbo_partner_id, {
		...
		OnDataLoading: (criteria) => {
			var empl_id = form.getValue(obj.cbo_empl_id)
			criteria.col_empl_id = empl_id;
		},
		OnDataLoaded : (result, options) => {
	...
	new fgta4slideselect(obj.cbo_collfuptype_id, {
		title: 'Pilih collfuptype_id',
		returnpage: this_page_id,
		api: $ui.apis.load_collfuptype_id,
		fieldValue: 'collfuptype_id',
		fieldValueMap: 'collfuptype_id',
		fieldDisplay: 'collfuptype_name',
		fields: [
			{mapping: 'collfuptype_id', text: 'collfuptype_id'},
			{mapping: 'collfuptype_name', text: 'collfuptype_name'},
		],
		OnDataLoading: (criteria) => {},
		OnDataLoaded : (result, options) => {
	...
	var fn_dataopened = async (result, options) => {
		...
		*/
		...
		form
			...
			.setValue(obj.cbo_collfuptype_id, record.collfuptype_id, record.collfuptype_name)
...
export function createnew() {
	form.createnew(async (data, options)=>{
		...
		data.collaccmtn_date = global.now()
		data.collaccmtn_nextactdate = global.now()
		data.collaccmtn_version = 0
		data.collaccmtn_iscommit = '0'
		...
		data.collfuptype_id = '0'
		data.collfuptype_name = '-- PILIH --'
...
function updaterecordstatus(record) {
	// apabila ada keperluan untuk update status record di sini
		...
		rec_commitby.html(record.collaccmtn_commitby);
		rec_commitdate.html(record.collaccmtn_commitdate);
...
function updatebuttonstate(record) {
	// apabila ada keperluan untuk update state action button di sini
		...
		if (record.collaccmtn_iscommit=="1") {
...
function updategridstate(record) {
	...
	var col_commit = 'collaccmtn_iscommit';
	updategriddata[col_commit] = record.collaccmtn_iscommit;	
...
function form_idsetup(options) {
	var objid = obj.txt_collaccmtn_id
...
async function form_datasaving(data, options) {
	...
	// options.skipmappingresponse = ['partnercontact_id', ];
...
async function btn_action_click(args) {
	...
	var txt_version = obj.txt_collaccmtn_version;
	var chk_iscommit = obj.chk_collaccmtn_iscommit;
