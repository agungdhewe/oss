/* API yang akan digunakan di program


	export const testapi = 'fgta/framework/formdev/nama-api'


kemdian, di program bisa diakses dengan

	$ui.apis.testapi

*/

export const load_periodemo_id = 'finact/master/periode/list-open'
export const load_empl_id = 'finact/collection/collector/list-collector'
export const load_dept_id = 'ent/organisation/dept/list-byuser'
export const load_doc_id = 'ent/organisation/docs/list'
export const load_partner_id = 'ent/affiliation/partner/list-byempl'
export const load_billout_id = 'finact/fin/billout/list-outstanding'


