...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.temprecv_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.temprecv_id
	}
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='temprecvdetil_descr') {

			td.innerHTML = `
				<div style="font-style: italic">${record.temprecvdetil_descr}</div>
				<div style="border-left: 3px solid #999; margin-top: 8px; margin-left: 1px; padding-left: 5px;">
					<div>${record.coa_name_or}</div>
					<div style="margin-left: 10px">${record.coa_name}</div>
				</div>
				<div style="border-left: 3px solid #f99; margin-top: 4px; margin-left: 1px; padding-left: 5px;">
					<div>${record.coa_name_tax}</div>
					<div style="margin-left: 10px">${record.coa_name}</div>
				</div>				
			`;
