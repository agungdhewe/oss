...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='temprecv_descr') {

			var status = '';
			if (record.temprecv_isposted=="1") {
				status += '<div class="inrow-id-block verified">&nbsp;</div>';
				status += '<div class="inrow-id-block posted">&nbsp;</div>';
			} else if (record.temprecv_isverified=="1") {
				status += '<div class="inrow-id-block verified">&nbsp;</div>';
			} else if (record.cashdiscount_iscommit=="1") {
				status += '<div class="inrow-id-block commit">&nbsp;</div>'
			} else {
			...
			td.innerHTML = `
				<div>
					<div class="inrow-id-block"><span style="margin-right: 3px">${record.temprecv_id}</span>${status}</div>
				</div>
				<div style="font-weight: bold">Agung N</div>
				<div style="font-style: italic">${record.temprecv_descr}</div> 
			`;
		}	
		// var mapping = td.getAttribute('mapping')
		// if (mapping=='id') {
