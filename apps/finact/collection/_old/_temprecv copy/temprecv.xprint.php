...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-temprecv.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':temprecv_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->temprecv_id = $row['temprecv_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_temprecv A where temprecv_id = :temprecv_id
		";
	}	
