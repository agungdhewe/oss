...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='cashdiscount_descr') {
			console.log(record);

			var status = '';
			if (record.cashdiscount_isposted=="1") {
				status += '<div class="inrow-id-block approved">&nbsp;</div>';
				status += '<div class="inrow-id-block posted">&nbsp;</div>';
			} else if (record.cashdiscount_isdeclined=="1") {
				status += '<div class="inrow-id-block rejected">&nbsp;</div>';
			} else if (record.cashdiscount_isapproved=="1") {
				status += '<div class="inrow-id-block approved">&nbsp;</div>';
			...
			td.innerHTML = `
				<div>
					<div class="inrow-id-block"><span style="margin-right: 3px">${record.cashdiscount_id}</span>${status}</div>
				</div>
				<div style="font-weight: bold">${record.partner_name}</div>
				<div style="font-style: italic">${record.cashdiscount_descr}</div> 
			`;
		}
	})
}
