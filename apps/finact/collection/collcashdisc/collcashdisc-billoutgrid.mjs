...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/billout-list`
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbilloutform', () => {
		$ui.getPages().ITEMS['pnl_editbilloutform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/billout-delete`
