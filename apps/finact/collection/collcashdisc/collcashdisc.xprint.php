...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-collcashdisc.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':collcashdisc_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->collcashdisc_id = $row['collcashdisc_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_collcashdisc A where collcashdisc_id = :collcashdisc_id
		";
	}	
