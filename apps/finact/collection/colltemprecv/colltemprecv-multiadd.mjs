...
export function LoadData(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria.partner_id = header_data.partner_id
...
function btn_apply_click() {
	...
	grd_list.IterateChecked({
		OnIterating : async (options) => {
			...
			args.data.colltemprecv_id = header_data.colltemprecv_id;
		...
		OnIterated: async (args) => {
			if (!stay) {
				$ui.getPages().show('pnl_editdetilgrid', ()=>{
					$ui.getPages().ITEMS['pnl_editdetilgrid'].handler.OpenDetil(header_data)
				})
			}
