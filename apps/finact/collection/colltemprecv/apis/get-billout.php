...
$API = new class extends colltemprecvBase {
	...
	public function execute($options) {
		...
		try {
			...
			$options->criteria->periodemo_id = '202110';
			...
			$stmt = $this->db->prepare("
					...
					, coalesce(B.billout_discp, 0) as billout_discp
					...
					, coalesce(B.billout_pphval, 0) as  billout_pphtopay
					, (select billout_isdiscvalue from view_collcashdisc where billout_id=A.billout_id and periodemo_id='$periodemo_id') as add_billout_isdiscvalue
					, (select billout_discp from view_collcashdisc  where billout_id=A.billout_id and periodemo_id='$periodemo_id') as add_billout_discp
					, (select billout_discval from view_collcashdisc where billout_id=A.billout_id and periodemo_id='$periodemo_id') as add_billout_discval
					from trn_billout A left join trn_colltargetbillout B on B.billout_id = A.billout_id
				" 
				. $where->sql 
				. " order by B.billout_datedue desc "
				. $limit
			);
