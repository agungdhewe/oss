...
const tbl_list = $('#pnl_editpartnerexcgrid-tbl_list');
const txt_title = $('#pnl_editpartnerexcgrid-title');
const pnl_control = $('#pnl_editpartnerexcgrid-control');
const btn_removechecked  = $('#pnl_editpartnerexcgrid-removechecked');
const btn_addrow = $('#pnl_editpartnerexcgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/partnerexc-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpartnerexcform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpartnerexcform', () => {
		$ui.getPages().ITEMS['pnl_editpartnerexcform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/partnerexc-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpartnerexcform', ()=>{
		$ui.getPages().ITEMS['pnl_editpartnerexcform'].handler.createnew(header_data)
	})	
}