 ...
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel account itemclass (mst_itemclass)
...
$API = new class extends itemclassBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_itemclassaccbudget A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.itemclassaccbudget_id, A.projectmodel_id, A.inquiry_accbudget_id, A.settl_coa_id, A.itemclass_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from mst_itemclassaccbudget A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
