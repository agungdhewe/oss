...
$API = new class extends itemmodelBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_itemmodel A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.itemmodel_id, A.itemmodel_name, A.itemmodel_descr, A.itemmodel_isintangible, A.itemmodel_issellable, A.itemmodel_isnonitem, A.itemmanage_id, A.dept_id, A.depremodel_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from mst_itemmodel A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
