...
export function LoadData(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.api = `${global.modulefullname}/list-getcoa`
		options.criteria.coaformat_id = header_data.coaformat_id;
...
function btn_apply_click() {
	...
	grd_list.IterateChecked({
		OnIterating : async (options) => {
			console.log(options)
			var apiurl = `${global.modulefullname}/coa-add`
			var args = {data: options.data, options: {}}

			args.data.coaformat_id = header_data.coaformat_id;
			args.data.coaformatset_id = header_data.coaformatset_id;
			try {
		...
		OnIterated: async (args) => {
			...
			if (!stay) {
				$ui.getPages().show('pnl_editaccountgrid', ()=>{
					$ui.getPages().ITEMS['pnl_editaccountgrid'].handler.OpenDetil(header_data)
				})
			}
