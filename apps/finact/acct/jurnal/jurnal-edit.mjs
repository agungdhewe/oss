...
const obj = {
	...
	chk_jurnal_ispayable: $('#pnl_edit-chk_jurnal_ispayable'),
	chk_jurnal_isreceivable: $('#pnl_edit-chk_jurnal_isreceivable')	
}
...
export async function init(opt) {
	...
	new fgta4slideselect(obj.cbo_curr_id, {
		...
		OnSelected: (value, display, record, args) => {		
			//obj.curr_islocal = record.curr_islocal;
			obj.cbo_curr_id.islocal = record.curr_islocal;
			//console.log(obj.cbo_curr_id);
			if (value!=args.PreviousValue ) {form.setValue(obj.txt_jurnal_valfrgrate, record.curr_rate)			
			}
		}
	})				
	...
	var fn_dataopened = async (result, options) => {
		...
		*/   
		...
		$ui.getPages().ITEMS['pnl_editdetilgrid'].handler.setSummary({
			jurnal_total: result.record.jurnal_validr,
			jurnal_variance: 0
		});
...
export function createnew() {
	form.createnew(async (data, options)=>{
		...
		data.jurnal_ispayable = '0'
		data.jurnal_isreceivable = '0'
		...
		data.jurnaltype_id = global.setup.jurnaltype_id
		data.jurnaltype_name = global.setup.jurnaltype_name
...
export function detil_open(pnlname) {
	...
	$ui.getPages().show(pnlname, () => {
		...
		header_data.partner_name = obj.cbo_partner_id.combo('getText');
		header_data.curr_name = obj.cbo_curr_id.combo('getText');
		header_data.curr_islocal = 	obj.cbo_curr_id.islocal;
...
function updaterecordstatus(record) {
	// apabila ada keperluan untuk update status record di sini
		...
		rec_postby.html(record.jurnal_postby);
		rec_postdate.html(record.jurnal_postdate);
...
function updatebuttonstate(record) {
	// apabila ada keperluan untuk update state action button di sini
		...
		btn_unpost.linkbutton(button_unpost_on ? 'enable' : 'disable');	
...
async function form_datasaving(data, options) {
	...
	// options.skipmappingresponse = ['coa_id', 'dept_id', 'partner_id', ];
...
function btn_print_click() {
	...
	var printurl = 'index.php/printout/' + window.global.modulefullname + '/jurnal.xprint?id=' + id;
...
async function btn_action_click(args) {
	...
	var chk_ispost = obj.chk_jurnal_ispost;
