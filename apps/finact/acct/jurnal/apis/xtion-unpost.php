...
$API = new class extends jurnalBase {
	...
	public function execute($id) {
		...
		try {
			...
			try {
				...
				$this->set_unpost_flag($id, $userdata);
				
				// $jurnaldoc = $this->create_jurnal_doc($id, $userdata);
				// $this->save_to_jurnal($jurnaldoc);
	...
	public function set_unpost_flag($id, $userdata) {
		try {
			...
			$sql = " 
				...
				set 
				jurnal_ispost = 0,
				jurnal_postby = :jurnal_postby,
				jurnal_postdate = :jurnal_postdate
				where
				jurnal_id = :id
			";
