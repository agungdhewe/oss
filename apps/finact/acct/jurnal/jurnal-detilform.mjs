...
export async function init(opt) {
	...
	new fgta4slideselect(obj.cbo_curr_id, {
		...
		OnSelected: (value, display, record, args) => {
			...
			if (value!=args.PreviousValue ) {
				form.setValue(obj.txt_jurnaldetil_valfrgrate, record.curr_rate)
				if (record.curr_islocal){
					form.setValue(obj.txt_jurnaldetil_valfrg, 0)
				}
			}			
		}
	})
	
	obj.txt_jurnaldetil_valfrg.numberbox({ onChange: (newvalue, oldvalue) => {  txt_jurnaldetil_valfrg_changed(newvalue, oldvalue); } })
	obj.txt_jurnaldetil_valfrgrate.numberbox({ onChange: (newvalue, oldvalue) => {  txt_jurnaldetil_valfrgrate_changed(newvalue, oldvalue); } })
...
export function createnew(hdata) {
	...
	form.createnew(async (data, options)=>{
		...
		data.jurnaldetil_descr = hdata.jurnal_descr
		data.jurnaldetil_valfrg = 0
		data.jurnaldetil_valfrgrate = hdata.jurnal_valfrgrate
		...
		data.dept_name = hdata.dept_name
		data.partner_id = hdata.partner_id
		data.partner_name = hdata.partner_name
		data.curr_id = hdata.curr_id
		data.curr_name = hdata.curr_name

		console.log(hdata);
		formformat_bycurr(hdata.curr_id, hdata);
...
function formformat_bycurr(curr_id, record) {
	if (record) {
		if (record.curr_islocal=='1') {
			comp_foreign.addClass('hide_foreign');
		} else {
			comp_foreign.removeClass('hide_foreign');
		}
	} else {		
		comp_foreign.addClass('hide_foreign');
	}
...
function txt_jurnaldetil_valfrg_changed(newvalue, oldvalue) {
	form_recalculate();
}


function txt_jurnaldetil_valfrgrate_changed(newvalue, oldvalue) {
	form_recalculate();
}


function form_recalculate(){
	var frg = parseFloat(form.getValue(obj.txt_jurnaldetil_valfrg));
	var rate = parseFloat(form.getValue(obj.txt_jurnaldetil_valfrgrate));
	var idr = frg * rate;
