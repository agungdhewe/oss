...
const tbl_list = $('#pnl_editdebetgrid-tbl_list');
const txt_title = $('#pnl_editdebetgrid-title');
const pnl_control = $('#pnl_editdebetgrid-control');
const btn_removechecked  = $('#pnl_editdebetgrid-removechecked');
const btn_addrow = $('#pnl_editdebetgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/debet-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdebetform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdebetform', () => {
		$ui.getPages().ITEMS['pnl_editdebetform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/debet-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdebetform', ()=>{
		$ui.getPages().ITEMS['pnl_editdebetform'].handler.createnew(header_data)
	})	
}