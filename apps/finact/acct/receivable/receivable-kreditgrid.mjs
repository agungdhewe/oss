...
const tbl_list = $('#pnl_editkreditgrid-tbl_list');
const txt_title = $('#pnl_editkreditgrid-title');
const pnl_control = $('#pnl_editkreditgrid-control');
const btn_removechecked  = $('#pnl_editkreditgrid-removechecked');
const btn_addrow = $('#pnl_editkreditgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/kredit-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editkreditform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editkreditform', () => {
		$ui.getPages().ITEMS['pnl_editkreditform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/kredit-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editkreditform', ()=>{
		$ui.getPages().ITEMS['pnl_editkreditform'].handler.createnew(header_data)
	})	
}