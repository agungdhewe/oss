...
/**
 * finact/acct/receivable/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header receivable (trn_jurnal)
 ...
 * tanggal 08/06/2021
 */
$API = new class extends receivableBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from view_jurnalar A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				jurnal_id, jurnaltype_id, jurnal_ref, periodemo_id, jurnal_date, jurnal_datedue, partner_id, jurnal_descr, jurnal_valfrg, curr_id, jurnal_valfrgrate, jurnal_validr, coa_id, dept_id, jurnalsource_id, jurnal_version, jurnal_iscommit, jurnal_commitby, jurnal_commitdate, jurnal_ispost, jurnal_postby, jurnal_postdate, jurnal_isclose, jurnal_isagingclose, _createby, _createdate, _modifyby, _modifydate 
				from view_jurnalar A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
