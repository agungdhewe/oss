...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-salesorder.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':salesorder_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->salesorder_id = $row['salesorder_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_salesorder A where salesorder_id = :salesorder_id
		";
	}	
