...
/**
 * finact/master/salesordertype/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header salesordertype (mst_salesordertype)
	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"search" => " A.salesordertype_id LIKE CONCAT('%', :search, '%') OR A.salesordertype_name LIKE CONCAT('%', :search, '%') "
				]
			);
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_salesordertype A" . $where->sql);
