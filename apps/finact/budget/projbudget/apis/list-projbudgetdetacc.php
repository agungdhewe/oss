...
$API = new class extends projbudgetBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from view_projbudgetdetacc A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				projbudget_id, projbudgetdet_id, projbudgetdet_descr, accbudget_id, accbudget_name
				from view_projbudgetdetacc A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
