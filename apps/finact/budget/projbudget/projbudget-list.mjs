...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='projbudget_descr') {
			console.log('format');
			var status = '';
			if (record.projbudget_isdeclined=="1") {
				status += '<div class="inrow-id-block rejected">&nbsp;</div>';
			} else if (record.projbudget_isapproved=="1") {
			...
			var descr = '';
			if (record.projbudget_descr.trim()!='') {
				descr = `<div style="font-style: italic; margin-bottom: 3px">${record.projbudget_descr}</div>`;
			}

			td.innerHTML = `
				<div>
					<div class="inrow-id-block"><span style="margin-right: 3px">${record.projbudget_name}</span>${status}</div>
				</div>
				${descr}
				<div style="font-size: 0.8em">${record.month_name} ${record.projbudget_year}</div>
