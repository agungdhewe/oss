...
const obj = {
	txt_corpbudget_id: $('#pnl_edit-txt_corpbudget_id'),
	txt_corpbudget_year: $('#pnl_edit-txt_corpbudget_year'),
	chk_corpbudget_iscommit: $('#pnl_edit-chk_corpbudget_iscommit'),
	txt_corpbudget_commitby: $('#pnl_edit-txt_corpbudget_commitby'),
	txt_corpbudget_commitdate: $('#pnl_edit-txt_corpbudget_commitdate'),
	chk_corpbudget_isapprovalprogress: $('#pnl_edit-chk_corpbudget_isapprovalprogress'),
	chk_corpbudget_isapproved: $('#pnl_edit-chk_corpbudget_isapproved'),
	txt_corpbudget_approveby: $('#pnl_edit-txt_corpbudget_approveby'),
	txt_corpbudget_approvedate: $('#pnl_edit-txt_corpbudget_approvedate'),
	chk_corpbudget_isdeclined: $('#pnl_edit-chk_corpbudget_isdeclined'),
	txt_corpbudget_declineby: $('#pnl_edit-txt_corpbudget_declineby'),
	txt_corpbudget_declinedate: $('#pnl_edit-txt_corpbudget_declinedate'),
	txt_corpbudget_notes: $('#pnl_edit-txt_corpbudget_notes'),
	txt_corpbudget_version: $('#pnl_edit-txt_corpbudget_version'),
...
export async function init(opt) {
	...
	form = new global.fgta4form(pnl_form, {
		primary: obj.txt_corpbudget_id,
		autoid: true,
		logview: 'mst_corpbudget',
...
export function createnew() {
	form.createnew(async (data, options)=>{
		...
		data.corpbudget_year = 0
		data.corpbudget_version = 0
...
function updaterecordstatus(record) {
	// apabila ada keperluan untuk update status record di sini
		...
		rec_commitby.html(record.corpbudget_commitby);
		rec_commitdate.html(record.corpbudget_commitdate);
		
		rec_approveby.html(record.corpbudget_approveby);
		rec_approvedate.html(record.corpbudget_approvedate);
		rec_declineby.html(record.corpbudget_declineby);
		rec_declinedate.html(record.corpbudget_declinedate);
...
function updatebuttonstate(record) {
	// apabila ada keperluan untuk update state action button di sini
		...
		if (record.corpbudget_isfirm=="1") {
			button_commit_on = false;
			button_uncommit_on = false;
			button_approve_on = false;
			button_decline_on = false;
			form.lock(true);	
		} else if (record.corpbudget_isdeclined=="1" || record.corpbudget_isuseralreadydeclined=="1") {
			button_commit_on = false;
			button_uncommit_on = true;
			button_approve_on = true;
			button_decline_on = false;
			form.lock(true);	
		} else if (record.corpbudget_isapproved=="1" || record.corpbudget_isuseralreadyapproved=="1") {
			button_commit_on = false;
			button_uncommit_on = false;
			button_approve_on = false;
...
function updategridstate(record) {
	...
	var col_commit = 'corpbudget_iscommit';
	updategriddata[col_commit] = record.corpbudget_iscommit;	
	
	var col_approveprogress = 'corpbudget_isapprovalprogress';
	var col_approve = 'corpbudget_isapprove'
	var col_decline = "corpbudget_isdeclined"
	updategriddata[col_approveprogress] = record.corpbudget_isapprovalprogress;
	updategriddata[col_approve] = record.corpbudget_isapproved;
	updategriddata[col_decline] = record.corpbudget_isdeclined;				
...
function form_viewmodechanged(viewmode) {
	...
	$ui.getPages().ITEMS['pnl_editdetilgrid'].handler.form_viewmodechanged(viewmode);
}

function form_idsetup(options) {
	var objid = obj.txt_corpbudget_id
...
function btn_print_click() {
	...
	var id = obj.txt_corpbudget_id.textbox('getValue');
	var printurl = 'index.php/printout/' + window.global.modulefullname + '/corpbudget.xprint?id=' + id;
...
async function btn_action_click(args) {
	...
	var docname = 'Corp Budget'
	var txt_version = obj.txt_corpbudget_version;
	var chk_iscommit = obj.chk_corpbudget_iscommit;
	
	var chk_isapprovalprogress = obj.chk_corpbudget_isapprovalprogress;	
	var chk_isapprove = obj.chk_corpbudget_isapproved;
	var chk_isdeclined = obj.chk_corpbudget_isdeclined;
