import { PasteFromSpreadsheet } from './corpbudget.xtion-paste.mjs'
...
const btn_summary = $('#pnl_editdetilgrid-btn_summary');
const btn_download = $('#pnl_editdetilgrid-btn_download');
const pnl_paste = $('#pnl_editdetilgrid-paste-box');
...
export async function init(opt) {
	...
	btn_summary.linkbutton({ onClick: () => { btn_summary_click() } });
	btn_download.linkbutton({ onClick: () => { btn_download_click() } });


	document.getElementById('pnl_editdetilgrid-paste-box').addEventListener('paste', (e) => {
		var clipboardData = e.clipboardData || window.clipboardData;
		var pastedData = clipboardData.getData('Text');
		e.stopPropagation();
		e.preventDefault();

		PasteFromSpreadsheet(header_data.corpbudget_id, pastedData, () => {
			pnl_paste.html('');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.corpbudget_id)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.corpbudget_id
	}
...
export function form_viewmodechanged(viewmode, param) {
	if (viewmode) {
		btn_summary.hide();
		pnl_paste.hide();
	} else {
...
function btn_summary_click() {
	$ui.ShowMessage("[QUESTION]Setiap baris dari coorporate budget akan ditimpa dengan data total jumlah budget departemen. Apakah anda yakin ?" , {
		yes: async () => {
			var apiurl = `${global.modulefullname}/xtion-summaryfromdept`
			var args = {
				id: header_data.corpbudget_id
			}
			...
			try {
				...
				if (result.success==true) {
					...
					$ui.ShowMessage("[ERROR] terjadi kesalahan");
				}
			} catch (err) {
				$ui.unmask();
...
function btn_download_click() {
	$ui.download(`${global.modulefullname}/xtion-download`, {
		corpbudget_id: header_data.corpbudget_id
	})
}