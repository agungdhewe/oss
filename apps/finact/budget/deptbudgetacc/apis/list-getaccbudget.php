...
$API = new class extends deptbudgetaccBase {
	...
	public function execute($options) {
		...
		try {
			...
			\FGTA4\utils\SqlUtility::setDefaultCriteria($options->criteria, 'dept_id', 1);
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				...
				[
					...
					"accbudget_include" => " (B.deptbudgetacc_id is null or A.accbudget_id=:accbudget_include) ",
					"dept_id" => '--'
				]
			);
			...
			$stmt = $this->db->prepare("
					select 
					count(*) as n 
					from mst_accbudget A left join mst_deptbudgetacc B on B.accbudget_id=A.accbudget_id and B.dept_id = :dept_id		
					" 
					. $where->sql
			);
