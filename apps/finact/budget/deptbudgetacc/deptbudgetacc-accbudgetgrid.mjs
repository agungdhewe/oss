...
const tbl_list = $('#pnl_editaccbudgetgrid-tbl_list');
const txt_title = $('#pnl_editaccbudgetgrid-title');
const pnl_control = $('.pnl_editaccbudgetgrid-control');
const btn_removechecked  = $('.pnl_editaccbudgetgrid-removechecked');
const btn_addrow = $('.pnl_editaccbudgetgrid-addrow');
const btn_addrowmulti = $('.pnl_editaccbudgetgrid-addrow-multi');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/accbudget-list`
		options.criteria['id'] = data.dept_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editaccbudgetform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editaccbudgetform', () => {
		$ui.getPages().ITEMS['pnl_editaccbudgetform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/accbudget-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editaccbudgetform', ()=>{
		$ui.getPages().ITEMS['pnl_editaccbudgetform'].handler.createnew(header_data)
	})	
}

function btn_addrowmulti_click() {
	$ui.getPages().show('pnl_accbudgetmultiadd', ()=>{
		$ui.getPages().ITEMS['pnl_accbudgetmultiadd'].handler.LoadData(header_data)
	})
}