...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=="deptbudgetrev_year") {
			td.innerHTML = `${record.month_name} ${record.deptbudgetrev_year}`;

		} else if (mapping=='deptbudgetrev_descr') {
			var status = '';
			if (record.deptbudgetrev_isdeclined=="1") {
				status += '<div class="inrow-id-block rejected">&nbsp;</div>';
			} else if (record.deptbudgetrev_isapproved=="1") {
			...
			td.innerHTML = `
				<div>
					<div class="inrow-id-block"><span style="margin-right: 3px">${record.deptbudgetrev_id}</span>${status}</div>
				</div>
				<div>${record.deptbudgetrev_descr}</div>
				<div style="font-style: italic">${record.dept_name}</div>
			`;
		}
	})
}
