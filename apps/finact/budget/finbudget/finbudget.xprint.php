...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-finbudget.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':finbudget_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->finbudget_id = $row['finbudget_id'];
	...
	function getSqlHeader() {
		return "
			...
			from mst_finbudget A where finbudget_id = :finbudget_id
		";
	}	
