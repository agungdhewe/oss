...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-deptbudget.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':deptbudget_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->deptbudget_id = $row['deptbudget_id'];
	...
	function getSqlHeader() {
		return "
			...
			from mst_deptbudget A where deptbudget_id = :deptbudget_id
		";
	}	
