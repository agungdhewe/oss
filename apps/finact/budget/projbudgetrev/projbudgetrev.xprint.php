...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-projbudgetrev.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':projbudgetrev_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->projbudgetrev_id = $row['projbudgetrev_id'];
	...
	function getSqlHeader() {
		return "
			...
			from mst_projbudgetrev A where projbudgetrev_id = :projbudgetrev_id
		";
	}	
