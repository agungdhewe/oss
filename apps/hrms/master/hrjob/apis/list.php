...
class DataList extends WebAPI {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_hrjob A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				hrjob_id, hrjob_name, hrjob_descr, hrjob_isdisabled, hrgrd_id, deptmodel_id, hrsection_id, _createby, _createdate, _modifyby, _modifydate 
				from mst_hrjob A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
