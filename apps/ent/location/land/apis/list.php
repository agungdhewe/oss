...
class DataList extends WebAPI {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_land A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				land_id, land_name, land_address, land_phone, land_email, land_isdisabled, land_geoloc, landtype_id, zone_id, city_id, partner_id, _createby, _createdate, _modifyby, _modifydate 
				from mst_land A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
