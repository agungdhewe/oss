...
const tbl_list = $('#pnl_editdelegategrid-tbl_list')
const txt_title = $('#pnl_editdelegategrid-title')
const pnl_control = $('#pnl_editdelegategrid-control')
const btn_removechecked  = $('#pnl_editdelegategrid-removechecked')
const btn_addrow = $('#pnl_editdelegategrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/delegate-list`
		options.criteria['id'] = data.auth_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdelegateform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdelegateform', () => {
		$ui.getPages().ITEMS['pnl_editdelegateform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/delegate-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdelegateform', ()=>{
		$ui.getPages().ITEMS['pnl_editdelegateform'].handler.createnew(header_data)
	})	
}