...
const pnl_control = $('#pnl_edittypegrid-control');
const btn_removechecked  = $('#pnl_edittypegrid-removechecked');
const btn_addrow = $('#pnl_edittypegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/type-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittypeform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittypeform', () => {
		$ui.getPages().ITEMS['pnl_edittypeform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/type-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittypeform', ()=>{
		$ui.getPages().ITEMS['pnl_edittypeform'].handler.createnew(header_data)
	})	
}