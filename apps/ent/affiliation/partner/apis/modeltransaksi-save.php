...
/**
 * ent/affiliation/partner/apis/modeltransaksi-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel modeltransaksi partner (mst_partner)
 *
...
$API = new class extends partnerBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\partner_modeltransaksiHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new partner_modeltransaksiHandler($data, $options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'partnertrxmodel_id', 'trxmodel_id', 'coa_id', 'unbill_coa_id', 'partner_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);
