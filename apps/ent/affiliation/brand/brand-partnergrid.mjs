...
const tbl_list = $('#pnl_editpartnergrid-tbl_list');
const txt_title = $('#pnl_editpartnergrid-title');
const pnl_control = $('#pnl_editpartnergrid-control');
const btn_removechecked  = $('#pnl_editpartnergrid-removechecked');
const btn_addrow = $('#pnl_editpartnergrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.brand_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/partner-list`
		options.criteria['id'] = data.brand_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpartnerform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpartnerform', () => {
		$ui.getPages().ITEMS['pnl_editpartnerform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/partner-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpartnerform', ()=>{
		$ui.getPages().ITEMS['pnl_editpartnerform'].handler.createnew(header_data)
	})	
}